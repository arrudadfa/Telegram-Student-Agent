...
class PropagandaService:
    ...
    def _should_send_to_chat(self, chat_id: int) -> bool:
        """Não envia propaganda em chat privado de quem já comprou o serviço."""
        if chat_id > 0 and payment_service.is_paid_user(chat_id):
            return False
        return True
    ...
    async def send_daily_propaganda(self):
        ...
        for group_id in ALLOWED_GROUP_IDS:
            if not self._should_send_to_chat(group_id):
                logger.info(f"Propaganda ignorada para usuário pagante {group_id}")
                continue
            try:
