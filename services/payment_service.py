...
class PaymentService:
    ...
    def _load_data(self):
        ...
        try:
            if os.path.exists(self.data_file):
                ...
                logger.info(f"Carregados {len(self.paid_users)} usuários pagantes")
    ...
    def _save_data(self):
        ...
        try:
            ...
            logger.info(f"Dados de pagamento salvos: {len(self.paid_users)} usuários pagantes")
        except Exception as e:
    ...
    async def verify_payment_for_user(self, user_id: int) -> bool:
        ...
        try:
            status = await mercadopago_service.check_payment_status(str(payment_id))
            logger.info(f"Status do pagamento {payment_id} para usuário {user_id}: {status}")
            
            # Se o pagamento foi aprovado, confirma
            if status == 'approved':
                if self.confirm_payment(user_id):
                    logger.info(f"Pagamento confirmado automaticamente para usuário {user_id}")
                    return True
                else:
